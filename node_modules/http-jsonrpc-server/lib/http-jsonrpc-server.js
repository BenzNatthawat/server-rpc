...
class RpcServer {
  ...
  async processRequest(request) {
    ...
    if (request.transId) {
      if (request.transId !== null && typeof request.transId !== 'number' && typeof request.transId !== 'string') {
        response.error = {
    ...
    if (request.rpcVer !== '3.0') {
      response.error = {
        code: consts.INVALID_REQUEST,
        message: 'Invalid rpcVer value',
      };
    } else if (!request.transId) {
      ...
      try {
        ...
        if (this.onRequestError) {
          this.onRequestError(err, request.transId);
        }
    ...
    if (this.onResult && response.result) {
      this.onResult(response.result, request.transId);
    }
    return response;
  }
}
